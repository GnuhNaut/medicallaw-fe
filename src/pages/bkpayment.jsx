...
const PaymentPage = () => {
  ...
  const handleDownloadQR = () => {
    // Truy cập canvas element thông qua id được gán cho QRCodeCanvas
    const canvas = document.getElementById('payment-qrcode-canvas');
    if (canvas) {
      try {
        const pngUrl = canvas
          .toDataURL("image/png") // Lấy data URL dạng PNG
          .replace("image/png", "image/octet-stream"); // Ép kiểu để trình duyệt tải về thay vì hiển thị
        let downloadLink = document.createElement("a");
        downloadLink.href = pngUrl;
        downloadLink.download = `HIMA2025_PaymentQR_${ticket_id}.png`; // Tên file tải về
        document.body.appendChild(downloadLink);
        downloadLink.click();
        document.body.removeChild(downloadLink);
      } catch (e) {
        console.error("Could not download QR Code", e);
        alert(t('paymentPage.errorDownloadQR', 'Không thể tải mã QR.')); // Thêm key dịch
      }
    } else {
        console.error("QR Code Canvas not found");
        alert(t('paymentPage.errorDownloadQR', 'Không thể tải mã QR.'));
    }
  };
        ...
        <div className="flex flex-col md:flex-row gap-8 md:gap-12">
          ...
          <div className="w-full md:w-1/2 flex flex-col p-4 md:p-6 border border-gray-200 rounded-xl bg-gray-50">
            ...
            <div className="relative p-2 border-2 border-dashed border-gray-300 rounded-lg self-center mb-4">
              {
                paymentData?.qr_code_data?.qrCode ? (
                  <QRCodeCanvas
                    value={paymentData?.qr_code_data?.qrCode} // Lấy raw data từ state hoặc dùng biến mẫu
                    size={240} // Kích thước QR code (pixel)
                    bgColor={"#ffffff"} // Màu nền
                    fgColor={"#000000"} // Màu QR
                    level={"L"} // Mức độ sửa lỗi ('L', 'M', 'Q', 'H')
                    includeMargin={true} // Có khoảng trắng viền hay không
                    id='payment-qrcode-canvas'
                />
                ) : (
                <img
                  id="qr-image"
                  src={'/assets/imgs/qrcodetp.png'} // Lấy URL QR từ state paymentData
                  alt="Payment QR Code"
            ...
            {/* Nút tải QR */}
            {
              ...
              >
                ...
                <span>{t('paymentPage.downloadQR', 'Tải xuống QR')}</span>
              </button> 
                : 
              <a
                id="download-qr-btn"
                href={'/assets/imgs/qrcodetp.png'}
